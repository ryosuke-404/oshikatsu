...
class _AddItineraryScreenState extends State<AddItineraryScreen> {
  ...
  void initState() {
    super.initState();
    _loadOshis();
    _addTodoItem(); // 初期状態で1つのToDo入力欄を追加
  }
  ...
  void _saveItinerary() {
    if (_formKey.currentState!.validate()) {
      final box = Hive.box<Itinerary>('itineraries');
      try {
        final newItinerary = Itinerary(
          ...
          oshiId: _selectedOshiId, // Save selected Oshi ID
          todoList: _todoControllers
              .where((c) => c.text.isNotEmpty)
              .map((c) => TodoItem(description: c.text))
              .toList(), // ToDoリストを保存
        );
        box.add(newItinerary);
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('create_itinerary_title'.tr()),
          actions: [
